...
class RegistryManager extends Object
{
    public function getModule($codename)
    {
        ...
        $r = $em->getRepository('Entities\Base\Module')->findOneBy(array('codename'=>$codename));
        return $r;
    }
    ...
    {
        $em = $this->getReader()->getEntityManager();
        $dql = 'SELECT u FROM Entities\Base\Module u ORDER BY u.codename';
        if ($asArray)
        {
    ...
    {
        $em = $this->getReader()->getEntityManager();
        $dql = 'SELECT u FROM Entities\Base\TreeNode u JOIN u.tree_type t WHERE t.codename = ?1';
    ...
    public function getActionDefinitions($asArray = false)
    {
        ...
        $dql = 'SELECT u,m FROM Entities\Base\ActionDefinition u JOIN u.module m WHERE u.is_widget = ?1 AND u.method <> ?2 AND m.codename <> ?3';
    ...
    {
        $em = $this->getReader()->getEntityManager();
        $dql = 'SELECT u FROM Entities\Base\Rule u ORDER BY u.codename';
    ...
    {
        $em = $this->getReader()->getEntityManager();
        $dql = 'SELECT u FROM Entities\Base\Role u';
    ...
    {
        $em = $this->getReader()->getEntityManager();
        $dql = 'SELECT u FROM Entities\Base\ActionDefinition u JOIN u.module m WHERE m.codename = ?1';
    ...
    public function existModule($filterArray)
    {
        ...
        $i = $em->getRepository('Entities\Base\Module')->findOneBy($filterArray);
