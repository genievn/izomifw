...
class MenuManager extends Object
{
    public function registerTreeType($codename = 'base.tree.menu', $parent = null)
    {
        ...
        $t = $em->getRepository('Entities\Base\TreeType')->findOneBy(array('codename'=>$codename));
        
        # if it doesn't exist create a new tree type
        if (!$t) {
            $t = new Entities\Base\TreeType();
            $t->title = 'Menu';
            $t->codename = $codename;
            $t->uuid = new_uuid();
        }
        ...
        {
            #find the parent
            $p = $em->getRepository('Entities\Base\TreeType')->findOneBy(array('codename'=>$parent));
